...
IMAGE_LABELS_DIR = './prepare_data/label'
...
def get_random_cached_bottlenecks(sess, image_lists, how_many, category,
  ...
  for unused_i in range(how_many):
    ...
    label_index = 0 # there is only one folder with images = 'multi_label'
...
def add_final_training_ops(class_count, final_tensor_name, bottleneck_tensor):
  ...
  with tf.name_scope('cross_entropy'):
    ...
    #   logits=logits, labels=ground_truth_input) # original
    cross_entropy = tf.nn.weighted_cross_entropy_with_logits(
        logits=logits, targets=ground_truth_input, pos_weight=6.0) #with weight
...
def add_evaluation_step(result_tensor, ground_truth_tensor):
  ...
  with tf.name_scope('accuracy'):
    with tf.name_scope('correct_prediction'):
      ...
      # But we have more ones (indicating multiple labels) in one row of result_tensor due to the multi_label classification
